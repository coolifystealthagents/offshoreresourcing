...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const url = `${base}/blog/${post.slug}`;
  const sources = details?.sources ?? (dailySep18Offa60Post ? dailyBlogSep18Offa60Details[dailySep18Offa60Post.slug].sources : []);
  const faqs = details && 'faqs' in details ? details.faqs : (dailySep18Offa60Post ? dailyBlogSep18Offa60Details[dailySep18Offa60Post.slug].faqs : []);
  const schema = {
