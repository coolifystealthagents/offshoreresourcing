...
How should a buyer review false-negative risk in an offshore candidate screening lane? Start with a defined unit and an acceptance rule, then inspect both accepted and rejected cases. For this topic, the unit is the **screening decision**. The minimum record should cover criterion, evidence observed, permitted clarification, outcome, reviewer, rule version, and reversal reason. Treat missing information as uncertainty, not evidence of success or failure.
...
The central failure mode here is a rejection caused by a proxy, missing note, inconsistent interpretation, or unavailable clarification rather than a job-related criterion. It matters because downstream reports inherit upstream classifications. A dashboard cannot correct a missing source, an ambiguous rule, or an unrecorded exception. Buyers should therefore review the evidence-producing process before relying on its totals.
...
For each screening decision, record criterion, evidence observed, permitted clarification, outcome, reviewer, rule version, and reversal reason. Separate observed facts from analysis and decisions. A source link or system identifier should make the observation reproducible without copying unnecessary personal information into a spreadsheet. Analysis may explain why a rule appears to apply; the authorized owner records the decision.
...
A candidate is rejected for a schedule answer recorded without a time zone. A sampled review finds that the stated hours actually overlap the required window, so the buyer corrects the rule and reopens the record. This example is not a universal rule. It shows why the record must preserve the original input, the applied criterion, uncertainty, and the buyer's correction. One anecdote can reveal a defect but cannot estimate its prevalence.
...
At the review point, choose expand, continue, narrow, repair, or stop. Document why, which evidence supports the choice, what remains unknown, and when it will be reconsidered. Where a viable need is established, the [candidate screening coordination service](/services/candidate-screening-coordination) provides a related administrative pathway; the buyer retains hiring, policy, access, and professional decisions.

## Buyer checklist

1. Define the screening decision, population, period, and decision owner.
